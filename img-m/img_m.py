...
class Parse:
    ...
    def separate_types_classifications(self, dict_biosamples):
        ...
        def save_specie(this_organism, this_data, dict_species):
            if this_organism not in dict_species:
                # Only the first id is registered
                dict_species.update({this_organism: this_data})
            else:
                current = dict_species[this_organism].copy()
                current[self.KEY_COUNT] = current[self.KEY_COUNT] + this_data[self.KEY_COUNT]
               ...
               (len(_organism) >= 2 and (re.search('[A-Z]{2}', _organism[0]) or re.search('[A-Z]{2}', _organism[1]) or _organism[1].lower() == 'x')):
                ...
                if len(_organism) >= 2 and re.search('^[A-Z]', organism) and (re.search('^sp[.]', _organism[1])):
                    if re.search('sp[.][\s][\'][a-z]+[\']', organism):
                        ...
                        save_specie(_organism, data, species)
    ...
    def create_taxonomic_rank_file(self, dict_species, dict_species_sp, dict_others):
        ...
        with open(self.TAXONOMIC_RANK_FILE, 'w', encoding = 'utf-8') as fw:
            fw.write('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' % (self.RANK_TAXON_ID,
                                                                                                                           self.KEY_COUNT,
...
def main(args):
    try:
        ...
        if option_extract:
            ...
            raw_file = 'taxontable_eukarya-20211113.txt'
